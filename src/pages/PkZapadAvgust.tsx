...
import {
  ...
  planFactBySegment, monthCompareBySegment, monthlyTrendBySegment, demandBySegment,
  workDone, workPlan, nextPlan,
} from '@/data/report-pk-zapad';
...
const PkZapadAvgust = () => {
  ...
  const [segPlanFact, setSegPlanFact] = useState<SegmentKey>('poligrafiya');
  const [segMonths, setSegMonths] = useState<SegmentKey>('poligrafiya');
  const [segTrends, setSegTrends] = useState<SegmentKey>('poligrafiya');
  const [segDemand, setSegDemand] = useState<SegmentKey>('poligrafiya');

  const scroll = (id: string) =>
    document.getElementById(id)?.scrollIntoView({ behavior: 'smooth' });

  const trendData = monthlyTrendBySegment[segTrends];
  const demandData = demandBySegment[segDemand];
      ...
      <div className="container mx-auto space-y-16 px-6 py-12">
        ...
        <Section id="trends" num="04" title="Тренды с начала работ" icon="ChartLine" sub="Проект в работе с июля 2026 — динамика по месяцам">
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <Card>
              <ChartTitle title="Рекламный бюджет, ₽" sub="Июль — Август"
              ...
              <ResponsiveContainer width="100%" height={260}>
                <LineChart data={trendData} margin={{ top: 30 }}>
                  ...
                  <Line type="monotone" dataKey="cost" name="Бюджет" stroke={NEON.cyan} strokeWidth={2.5} dot={{ r: 5 }} connectNulls={false}>
            ...
            <Card>
              <ChartTitle title="Заявки, ед." sub="Июль — Август"
              ...
              <ResponsiveContainer width="100%" height={260}>
                <LineChart data={trendData} margin={{ top: 30 }}>
                  ...
                  <Line type="monotone" dataKey="uniq" name="Заявки" stroke={NEON.violet} strokeWidth={2.5} dot={{ r: 5 }} connectNulls={false}>
            ...
            <Card>
              <ChartTitle title="Стоимость заявки, ₽" sub="Июль — Август"
              ...
              <ResponsiveContainer width="100%" height={260}>
                <LineChart data={trendData} margin={{ top: 30 }}>
                  ...
                  <YAxis stroke={axisColor} fontSize={11} />
                  <Tooltip contentStyle={tipStyle} formatter={(v: number) => `${fmt(v)} ₽`} />
                  <Line type="monotone" dataKey="costUniq" name="Стоимость заявки" stroke={NEON.amber} strokeWidth={2.5} dot={{ r: 5 }} connectNulls={false}>
            ...
            <Card>
              <ChartTitle title="Чистые заявки, ед." sub="Июль — Август"
              ...
              <ResponsiveContainer width="100%" height={260}>
                <LineChart data={trendData} margin={{ top: 30 }}>
                  ...
                  <YAxis stroke={axisColor} fontSize={11} />
                  <Tooltip contentStyle={tipStyle} />
                  <Line type="monotone" dataKey="clean" name="Чистые заявки" stroke={NEON.lime} strokeWidth={2.5} dot={{ r: 5 }} connectNulls={false}>
            ...
            <Card>
              <ChartTitle title="Стоимость чистой заявки, ₽" sub="Июль — Август"
              ...
              <ResponsiveContainer width="100%" height={260}>
                <LineChart data={trendData} margin={{ top: 30 }}>
                  ...
                  <YAxis stroke={axisColor} fontSize={11} />
                  <Tooltip contentStyle={tipStyle} formatter={(v: number) => `${fmt(v)} ₽`} />
                  <Line type="monotone" dataKey="costClean" name="Стоимость чист. заявки" stroke={NEON.violet} strokeWidth={2.5} dot={{ r: 5 }} connectNulls={false}>
            ...
            <Card>
              <ChartTitle title="Квал. заявки, ед." sub="Июль — Август"
              ...
              <ResponsiveContainer width="100%" height={260}>
                <LineChart data={trendData} margin={{ top: 30 }}>
                  ...
                  <YAxis stroke={axisColor} fontSize={11} />
                  <Tooltip contentStyle={tipStyle} />
                  <Line type="monotone" dataKey="qual" name="Квал. заявки" stroke={NEON.amber} strokeWidth={2.5} dot={{ r: 5 }} connectNulls={false}>
            ...
            <Card>
              <ChartTitle title="Стоимость квал. заявки, ₽" sub="Июль — Август"
              ...
              <ResponsiveContainer width="100%" height={260}>
                <LineChart data={trendData} margin={{ top: 30 }}>
                  ...
                  <YAxis stroke={axisColor} fontSize={11} />
                  <Tooltip contentStyle={tipStyle} formatter={(v: number) => `${fmt(v)} ₽`} />
                  <Line type="monotone" dataKey="costQual" name="Стоимость квал. заявки" stroke={NEON.neg} strokeWidth={2.5} dot={{ r: 5 }} connectNulls={false}>
          ...
          <Card className="mt-6 border-primary/30">
            ...
            <p className="text-sm leading-relaxed text-foreground/90">
              Август стал сильным месяцем по обоим направлениям. В «Полиграфии» количество заявок выросло
              до <b>84</b> при плане 80, а стоимость заявки снизилась до <b>2 408 ₽</b> против плановых 2 500 ₽ —
              рекламный бюджет освоен практически точно по плану. В «Упаковке» результат ещё заметнее: число заявок
              увеличилось с 39 в июле до <b>66 в августе</b>, при этом стоимость заявки снизилась до <b>2 962 ₽</b>,
              а доля чистых заявок выросла до <b>86,36%</b> — лучший показатель среди направлений. По «Полиграфии»
              количество квалифицированных заявок осталось стабильно высоким — <b>24</b>, полностью выполнив план.
              Рост объёма трафика в «Упаковке» — хороший сигнал для дальнейшего перераспределения бюджета в пользу
              этого направления в сентябре.
            </p>
          </Card>
        </Section>
        ...
        <Section id="demand" num="06" title="Спрос" icon="Search" sub="Число запросов по Wordstat: 2024 / 2025 / 2026">
          <Card>
            <SegmentTabs value={segDemand} onChange={setSegDemand} />
            ...
            <ResponsiveContainer width="100%" height={320}>
              <LineChart data={demandData} margin={{ top: 30 }}>
                <CartesianGrid strokeDasharray="3 3" stroke={isLight ? "hsl(0,0%,90%)" : "hsl(0,0%,20%)"} />
                <XAxis dataKey="m" stroke={axisColor} fontSize={12} />
                <YAxis stroke={axisColor} fontSize={11} />
                <Tooltip contentStyle={tipStyle} />
            ...
            <p className="mt-4 text-sm leading-relaxed text-foreground/90">
              {segDemand === 'poligrafiya'
                ? 'Спрос на полиграфию в 2026 году держится ниже уровня прошлых лет, но летом заметна стабилизация: в июле-августе показатель почти не изменился (7 960 → 8 053 запроса), в отличие от резкого весеннего спада. Традиционно пик спроса приходится на конец года — стоит учитывать это при планировании бюджета на ноябрь-декабрь.'
                : 'По направлению «Упаковка» в 2026 году впервые за долгое время спрос летом обгоняет показатели прошлых лет: в августе зафиксировано 7 007 запросов — выше, чем в августе 2024 и 2025 годов. Это подтверждает своевременность решения нарастить бюджет на упаковку и говорит в пользу дальнейшего масштабирования направления в сентябре.'}
            </p>
          </Card>
        </Section>
