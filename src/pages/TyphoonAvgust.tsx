...
import {
  ...
  workDone, workPlan, nextPlanMain, nextPlanQuiz,
} from '@/data/report-typhoon';
...
const TyphoonAvgust = () => {
  ...
  const [showVals, setShowVals] = useState({ cost: false, leads: false, cpl: false, clean: false, ccpl: false, qual: false, cql: false, demand: false });
  const [nextPlanTab, setNextPlanTab] = useState<NextPlanTab>('main');

  const scroll = (id: string) =>
    document.getElementById(id)?.scrollIntoView({ behavior: 'smooth' });

  const nextPlanData = nextPlanTab === 'main' ? nextPlanMain : nextPlanQuiz;
      ...
      <div className="container mx-auto space-y-16 px-6 py-12">
        ...
        <Section id="trends" num="04" title="Тренды с начала работ" icon="ChartLine" sub="Проект в работе с апреля 2026 — динамика по месяцам">
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <Card>
              ...
              <ResponsiveContainer width="100%" height={260}>
                <LineChart data={monthlyTrend} margin={{ top: 30 }}>
                  ...
                  <YAxis stroke={axisColor} fontSize={11} />
                  <Tooltip contentStyle={tipStyle} formatter={(v: number) => `${fmt(v)} ₽`} />
                  <Line type="monotone" dataKey="costLead" name="Стоимость заявки" stroke={NEON.amber} strokeWidth={2.5} dot={{ r: 3 }} connectNulls={false}>
                    {showVals.cpl && <LabelList dataKey="costLead" content={<ValueLabel fill={NEON.amber} isLight={isLight} />} />}
                  </Line>
                </LineChart>
              </ResponsiveContainer>
            </Card>
            ...
            <Card>
              ...
              <ResponsiveContainer width="100%" height={260}>
                <LineChart data={monthlyTrend} margin={{ top: 30 }}>
                  ...
                  <YAxis stroke={axisColor} fontSize={11} />
                  <Tooltip contentStyle={tipStyle} formatter={(v: number) => `${fmt(v)} ₽`} />
                  <Line type="monotone" dataKey="costClean" name="Стоимость чист. заявки" stroke={NEON.violet} strokeWidth={2.5} dot={{ r: 3 }} connectNulls={false}>
                    {showVals.ccpl && <LabelList dataKey="costClean" content={<ValueLabel fill={NEON.violet} isLight={isLight} />} />}
                  </Line>
                </LineChart>
              </ResponsiveContainer>
            </Card>
            ...
            <Card>
              <ChartTitle title="Квал. заявки, ед." sub="Апрель — Сентябрь"
              ...
              <ResponsiveContainer width="100%" height={260}>
                <LineChart data={monthlyTrend} margin={{ top: 30 }}>
                  ...
                  <YAxis stroke={axisColor} fontSize={11} />
                  <Tooltip contentStyle={tipStyle} />
                  <Line type="monotone" dataKey="qual" name="Квал. заявки" stroke={NEON.amber} strokeWidth={2.5} dot={{ r: 3 }} connectNulls={false}>
            ...
            <Card>
              <ChartTitle title="Стоимость квал. заявки, ₽" sub="Апрель — Сентябрь"
                action={<ValueToggle show={showVals.cql} setShow={(v) => setShowVals((s) => ({ ...s, cql: v }))} />} />
              <ResponsiveContainer width="100%" height={260}>
                <LineChart data={monthlyTrend} margin={{ top: 30 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke={isLight ? "hsl(0,0%,90%)" : "hsl(0,0%,20%)"} />
                  <XAxis dataKey="m" stroke={axisColor} fontSize={12} />
                  <YAxis stroke={axisColor} fontSize={11} />
                  <Tooltip contentStyle={tipStyle} formatter={(v: number) => `${fmt(v)} ₽`} />
                  <Line type="monotone" dataKey="costQual" name="Стоимость квал. заявки" stroke={NEON.neg} strokeWidth={2.5} dot={{ r: 3 }} connectNulls={false}>
                    {showVals.cql && <LabelList dataKey="costQual" content={<ValueLabel fill={NEON.neg} isLight={isLight} />} />}
                  </Line>
                </LineChart>
              </ResponsiveContainer>
            </Card>
          ...
          <Card className="mt-6 border-primary/30">
            ...
            <p className="text-sm leading-relaxed text-foreground/90">
              Август по сравнению с июлем показал снижение объёма трафика при заметном росте его качества:
              бюджет сократился с <b>482 554 ₽ до 344 540 ₽</b>, а число заявок — с 58 до 48, при этом стоимость
              заявки снизилась с 8 320 ₽ до <b>7 178 ₽</b>. Доля чистых заявок выросла с 50% до 52,08%, а конверсия
              из заявки в квалифицированную — с 48,28% до <b>56%</b>. В результате количество квал. заявок осталось
              на уровне июля (14 шт.), но стоимость квалифицированной заявки снизилась почти в полтора раза —
              с 34 468 ₽ до <b>24 610 ₽</b>. Такая динамика говорит об успешной чистке семантики и оптимизации
              кампаний: несмотря на меньший бюджет, воронка стала эффективнее на всех этапах.
            </p>
          </Card>
        </Section>
        ...
        {/* 6. СПРОС */}
        <Section id="demand" num="06" title="Спрос" icon="Search" sub="Число запросов по Wordstat: январь 2025 — август 2026">
          <Card>
            ...
            <ResponsiveContainer width="100%" height={320}>
              <LineChart data={demand} margin={{ top: 30 }}>
                <CartesianGrid strokeDasharray="3 3" stroke={isLight ? "hsl(0,0%,90%)" : "hsl(0,0%,20%)"} />
                <XAxis dataKey="m" stroke={axisColor} fontSize={11} interval={1} />
                <YAxis stroke={axisColor} fontSize={11} />
                <Tooltip contentStyle={tipStyle} />
                <Line type="monotone" dataKey="v" name="Запросов" stroke={NEON.cyan} strokeWidth={2.5} dot={{ r: 3 }}>
                  {showVals.demand && <LabelList dataKey="v" content={<ValueLabel fill={NEON.cyan} isLight={isLight} />} />}
                </Line>
              </LineChart>
            </ResponsiveContainer>
            <p className="mt-4 text-sm leading-relaxed text-foreground/90">
              Спрос по направлению нестабилен, но с явной положительной динамикой к текущему моменту: минимум
              был зафиксирован в мае 2025 (133 запроса), после чего начался рост с пиком в сентябре 2025
              (267 запросов). В 2026 году спрос держится на более высоком базовом уровне, чем в начале 2025 —
              август 2026 показал 239 запросов против 161 годом ранее, то есть рост почти на 48%. Сезонного
              провала в летние месяцы 2026 года практически не наблюдается, что говорит о растущем интересе
              к направлению — это хорошая база для увеличения бюджета в осенний период.
            </p>
          </Card>
        </Section>
        ...
        <Section id="nextplan" num="07" title="План на новый месяц" icon="Flag" sub="Плановые показатели на сентябрь 2026">
          <Card>
            <Tabs value={nextPlanTab} onValueChange={(v) => setNextPlanTab(v as NextPlanTab)} className="mb-5">
              <TabsList>
                <TabsTrigger value="main" className="gap-1.5">
                  <Icon name="Globe" size={14} /> Основной сайт
                </TabsTrigger>
                <TabsTrigger value="quiz" className="gap-1.5">
                  <Icon name="ListChecks" size={14} /> Квиз
                </TabsTrigger>
              </TabsList>
            </Tabs>
            <div className="overflow-x-auto">
